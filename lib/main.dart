...
class _GameScreenState extends State<GameScreen> with TickerProviderStateMixin {
  ...
  @override
  void initState() { super.initState(); _reset(); _loop = AnimationController(vsync: this, duration: const Duration(days: 1))..addListener(_update); _loop.forward(); }
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (ctx, cons) {
      ...
      return Scaffold(
        ...
        body: Stack(children: [
          CustomPaint(size: size, painter: GamePainter(shipPos, angle, planet, traj, orbit, engineOn && throttle > 0, cam, zoom, center)),
  ...
  Widget _controls() {
    return Container(width: 150, padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.black54, borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.white24)),
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        _sliderRow('THRUST', throttle, 0, 1, (v) => throttle = v, fuel > 0 ? Colors.orange : Colors.grey, Icons.rocket_launch),
        const SizedBox(height: 8),
        _sliderRow('ROTATE', rotInput, -1, 1, (v) => rotInput = v, Colors.cyan, Icons.rotate_right),
      ]));
  }
  ...
  Widget _sliderRow(String label, double val, double min, double max, ValueChanged<double> onChanged, Color color, IconData icon) {
    return StatefulBuilder(builder: (ctx, setSt) => Row(children: [
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('$label ${((val - min) / (max - min) * 100).abs().toInt()}%', style: const TextStyle(fontSize: 9, color: Colors.white70)),
        Slider(value: val, min: min, max: max, onChanged: (v) { setSt(() => onChanged(v)); setState(() {}); }, activeColor: color),
      ])),
    ]));
  }
  ...
  Widget _settingsPanel() => SafeArea(child: Column(children: [
    ...
    Expanded(child: SingleChildScrollView(padding: const EdgeInsets.all(12), child: Wrap(spacing: 12, runSpacing: 12, children: [
      _setSlider('Planet Mass', physics.planetMass, 5000, 200000, (v) => physics.planetMass = v, Colors.purple, Icons.public),
      _setSlider('Thrust Force', physics.thrustForce, 50, 1200, (v) => physics.thrustForce = v, Colors.orange, Icons.rocket),
      _setSlider('Rotation Speed', physics.rotationSpeed, 0.5, 8, (v) => physics.rotationSpeed = v, Colors.cyan, Icons.rotate_right),
      _setSlider('Rotation Lag', physics.rotationLag, 0, 0.95, (v) => physics.rotationLag = v, Colors.yellow, Icons.speed),
      _setSlider('Space Drag', physics.drag, 0.99, 1.0, (v) => physics.drag = v, Colors.blue, Icons.air),
      _setSlider('Planet Radius', physics.planetRadius, 20, 150, (v) => physics.planetRadius = v, Colors.green, Icons.circle),
      _setSlider('Ship Radius', physics.shipRadius, 5, 25, (v) => physics.shipRadius = v, Colors.grey, Icons.rocket_launch),
      _setSlider('Traj Steps', physics.trajSteps.toDouble(), 50, 800, (v) => physics.trajSteps = v.toInt(), Colors.teal, Icons.timeline),
    ]))),
  ]));

  Widget _setSlider(String label, double val, double min, double max, ValueChanged<double> onChanged, Color color, IconData icon) => StatefulBuilder(builder: (ctx, setSt) => Container(width: 260, padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.black45, borderRadius: BorderRadius.circular(10), border: Border.all(color: color.withOpacity(0.3))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
...
class GamePainter extends CustomPainter {
  final Offset shipPos; final double angle; final Offset planet; final List<Offset> traj; final List<Offset> orbit; final bool engineOn; final Offset cam; final double zoom; final Offset screenCenter;
  GamePainter(this.shipPos, this.angle, this.planet, this.traj, this.orbit, this.engineOn, this.cam, this.zoom, this.screenCenter);
  ...
  void paint(Canvas c, Size s) {
    ...
    for (int i = 0; i < 300; i++) {
      final sx = r.nextDouble() * 4000 - 2000;
      final sy = r.nextDouble() * 4000 - 2000;
      c.drawCircle(Offset(sx, sy), r.nextDouble() * 1.5 + 0.5, star);
    }

    // Orbit ellipse path
    if (orbit.length > 2) {
      final orbitPaint = Paint()..color = Colors.cyan.withOpacity(0.15)..style = PaintingStyle.stroke..strokeWidth = 2 / zoom;
      final path = Path();
      path.moveTo(orbit[0].dx, orbit[0].dy);
      for (var p in orbit.skip(1)) path.lineTo(p.dx, p.dy);
      path.close();
      c.drawPath(path, orbitPaint);
    }

    // Gravity well rings
    final gPaint = Paint()..color = Colors.purple.withOpacity(0.08)..style = PaintingStyle.stroke..strokeWidth =
 1 / zoom;
...
double lerpAngle(double a, double b, double t) { var d = b - a; while (d > pi) d -= 2 * pi; while (d < -pi) d += 2 * pi; return a + d * t; }
